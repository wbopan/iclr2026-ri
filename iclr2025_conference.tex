...
Therefore, a good metric of refusal-based self-knowledge should have the following properties: (1) quantify model self-knowledge regarding refusals, (2) remain consistent even when a model exhibits different refusal rates, and (3) be derived directly from answers of a blackbox LLM.
...
To understand the intuition behind the definition, we say a model is \emph{perfect-refusal-self-aware} when the probability of refusal is monotonically increasing with error probability, resulting in the model being more likely to refuse when a question becomes more difficult.
...
While overall refusal rates can be adjusted through input context or model preference learning, the discriminative capability is more robust and consistent. 
...
To infer the correlation between two latent continuous variables from binary observations, we propose a principled two-pass evaluation process. 
...
\paragraph{Baseline Metrics} We compare RI with five metrics from SimpleQA and HalluLens that are used for measuring refusal-based self-knowledge, shown in Table~\ref{tab:baseline_metrics}, including Correct Answer Rate, Refusal Rate, Correct given Attempted (C/A), F-score, and Weighted Score. 
...
We demonstrate that: (1) the Refusal Index conceptualizes and evaluates the model's refusal-based self-knowledge with an \emph{accuracy-refusal curve}, and (2) the two-pass evaluation returns consistent RI across different refusal rates.
...
By varying the overall refusal tendency, each model exhibits a trade-off between correct answers and refusal rate. 
...
In summary, the Refusal Index does not simply reward higher accuracy or lower refusal rates, but captures the level of rank calibration in refusal decisions, distinct from all previous metrics.
...
As shown in Table~\ref{tab:qwen25_simpleqa_prompt_strategies}, while increasing refusal rates increases the correct rate in answered questions (increasing C/A), their RI remains stable and far from perfection. 
...
Results show that RI values for PreciseWiki are similar to those of SimpleQA, and models generally have good ability to identify and avoid counterfactual context. However, when ground truth becomes unavailable, models are showing much worse self-knowledge. 
This suggests that models' self-knowledge may rely on existing partial information on the answer; they make poorer knowledge-aware refusals when the answer never appears in training data or context.
