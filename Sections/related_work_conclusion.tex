...
In this work, we introduced Refusal Index, a novel metric to address the unreliability of existing factuality evaluations for LLMs, which are often skewed by prompt design and refusal rate. RI provides a principled, calibration-based measure of knowledge-aware refusal by evaluating the alignment between its refusal to answer and the intrinsic difficulty of a question.  RI not only enables fairer model comparisons but also provides valuable insights for developing better-calibrated and more trustworthy AI systems.